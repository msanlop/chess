...
exports.move =
  exports.getAllowedMovesForPieceAtCoordinate =
  exports.isGameFinished =
  exports.getPiece =
  exports.startingGameState =
  exports.starterPosition =
  exports.STARTING_TIME =
  exports.BOARD_SIZE =
    void 0;
...
const hasNoAllowedMoves = (state, player) => {
  const whitePieces = state.board
    .map((val, index) => {
      return {
        ...val,
        x: index % exports.BOARD_SIZE,
        y: Math.floor(index / exports.BOARD_SIZE),
      };
    })
    .filter((v) => v !== null && v.color === player);
  for (const p of whitePieces) {
    if (
      (0, exports.getAllowedMovesForPieceAtCoordinate)(
        { x: p.x, y: p.y },
        state
      ).some((b) => b)
    ) {
...
const checkIfStepInDirectionAllowedForPiece = (
  ...
  if (obstaclePiece && obstaclePiece !== null) {
    ...
    //pawn attacks on the front
      ...
      movesRef[newCoords.x + exports.BOARD_SIZE * newCoords.y] =
        direction[0] !== 0;
      return null;
    } else {
