...
export const initAndReturnSocket = (server) => {
    ...
    const socketServerLog = (instanceId:string, content:string) => console.log((new Date).toISOString() + " -- " + padId(instanceId), ": " + content)
    ...
    const getGameInstanceOfPlayer = (token: string) : (GameInstance | undefined) => {
        ...
        if(!instaceId){
        socketServerLog("XX", "ERROR : PLAYER " + token + " NOT ASSIGNED TO A GAME");
        return;
        }
        const gameInstance = gameInstances.get(instaceId)
        if(!gameInstance){
        socketServerLog(instaceId, "ERROR : GAMEINSTANCE " + instaceId + " NOT DEFINED");
        return;
        }
        return gameInstance;
    }
    ...
    */
    io.on('connection', (socket) => {
        ...
        logAndEmit({
            ...
            player:color,
            socketId: socketId,
            content: color + " moved from " + coordinateToAlgebraic(from) + " to " + coordinateToAlgebraic(to)
        })
